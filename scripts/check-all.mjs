...
// -----------------------------------------------------------------------------
//   ・「約束と実装の突合」(verify-promises.mjs) … 部品検査には無い
//   ・「約束と実装の突合の自己試験」(selftest-promises.mjs) … 部品検査には無い
...
//
// -----------------------------------------------------------------------------
// 🚨🚨 2026-09-04 に直した所: **この一覧が嘘を配っていた**
// -----------------------------------------------------------------------------
//   ここには「書き込みの枠」について
//     『最終検査・製品検査にはあるが、部品検査・司令塔③には無い。移していない。』
//   と書いてあったが、**現物は在った**(実測: 4アプリで md5 6f896edc 一致・
//   走らせて 終了値0「繰り返し保管庫へ書く処理は見つかりませんでした」)。
//   説明だけが古く、段に載っていなかったので **一度も走っていなかった**。
//   → 段に載せた(15段目)。さらに下の verifyStepInventory() が
...
const STAGES = [
  ...
  },
  {
    ...
    steps: [
      { title: '自己試験+判定', cmd: [node, S('verify-watchdogs-read-code.mjs')] },
      // 🚨 2026-09-04: 「部品検査には無い/移していない」と書いた覚え書きが 1本ぶん嘘だった。
      //   人が書いた一覧は必ず古くなるので、機械で突き合わせる。
      { title: '「この段は無い」の説明が本当か', fn: verifyStepInventory },
    ],
  },
  {
  ...
  {
    id: 14, name: '消す印（消したのに戻ってくる）',
    steps: [
      // 2026-09-01 清水さん「ボタン連打しても休みとか他が戻ってこないから、ミスしても直せないよ」
      //   merge:true は **送らなかった鍵を消さない**。印が無いと焼き付いて戻せなくなる。
      //   実測 2026-09-02: 同じ形が 4アプリで 13件あった。
      { title: '見張り自身の試験(わざと壊す)', cmd: [node, S('verify-delete-mark.mjs'), '--selftest'] },
      { title: '判定', cmd: [node, S('verify-delete-mark.mjs')] },
    ],
  },
  {
    // ✍ 2026-09-04: 現物(scripts/verify-write-budget.mjs)は在ったのに、
    //   部品検査と司令塔③だけ **段に載っていなかった**(頭の一覧は「移していない」と書いていた=嘘)。
    //   md5 は4アプリとも 6f896edc で同じ。走らせると 終了値0（＝載せても赤は増えない）。
    id: 15, name: '書き込みの枠（同じ書類へ何度も書く形が無いか）',
    steps: [{ title: '判定', cmd: [node, S('verify-write-budget.mjs')] }],
  },
  {
    // 🧪 2026-09-04: 「エミュレータで確かめた」が成り立っていたか。
    //   司令塔③の連絡ポータル(?renraku=1)だけ 8080/9099 が焼き込まれたまま残っていた
    //   (③本体は 2026-08-30 に直っていた＝**片方だけ直した**形)。見張りが無かった。
    //   ⚠部品検査は環境変数から読んでいる(VITE_FIRESTORE_EMULATOR_PORT)ので、いまは緑。
...
// -----------------------------------------------------------------------------
// 🚨🚨 「この段は無い」と書いた説明が、実は在る物を指していないか（2026-09-04）
// -----------------------------------------------------------------------------
//   実測: このファイルの頭に「部品検査には無い/移していない」と書いてあった物のうち
//     verify-write-budget.mjs は **在った**(4アプリで md5 一致・走らせて 終了値0)。
//   人が書いた覚え書きは必ず古くなる。だから機械で突き合わせる:
//     ①「無い」と書いてある物の現物が在ったら赤（＝説明が嘘）
//     ② 拾える行が0件でも赤（＝黙って何も見ない見張りにしない）
//   ⚠ 拾うのは **最初の import より上** の覚え書きだけ。全文を拾うと、
//     この見張り自身の試験の文字や、直した経緯の説明で赤くなる(実測 2026-09-04)。
// -----------------------------------------------------------------------------
function stepInventory(srcOverride = null, exists = null) {
  const src = srcOverride != null ? srcOverride : fs.readFileSync(fileURLToPath(import.meta.url), 'utf8');
  const has = exists || ((f) => fs.existsSync(path.join(ROOT, 'scripts', f)));
  const head = src.split(/^import /m)[0];
  const claimedMissing = [...head.matchAll(/\(([\w.-]+\.mjs)\)[^\n]*(?:現物が無い|移していない|部品検査に(?:は)?無い)/g)].map((m) => m[1]);
  const problems = [];
  for (const f of new Set(claimedMissing)) {
    if (has(f)) problems.push(`「${f} は現物が無い/移していない」と書いてあるが、scripts/${f} は **在る**（説明が古い＝嘘を配っている）`);
  }
  return { claimedMissing: [...new Set(claimedMissing)], problems };
}

function verifyStepInventory() {
  // 🧪 まず見張り自身を試す。わざと嘘の説明を食わせて赤になるか。
  const FAKE = '// ・「うそ」(zzz-fake.mjs) … 部品検査には無い\nimport x from "y";\n';
  const t1 = stepInventory(FAKE, (f) => f === 'zzz-fake.mjs');
  const t2 = stepInventory(FAKE, () => false);
  console.log(`  ${t1.problems.length === 1 ? '✅' : '❌'} 🚨「無い」と書いた物の現物が在ったら赤にする（試験）`);
  console.log(`  ${t2.problems.length === 0 ? '✅' : '❌'} 本当に無い物は赤にしない（試験）`);
  if (t1.problems.length !== 1 || t2.problems.length !== 0) {
    console.log('  ❌ 見張り自身が壊れています。判定は出しません。');
    return false;
  }
  const r = stepInventory();
  if (!r.claimedMissing.length) {
    console.log('  ❌ 「現物が無い」と書いた行を1つも拾えませんでした（数え方が壊れている疑い＝何も見ていない）。');
    return false;
  }
  console.log(`  ・「無い」と書いてある物: ${r.claimedMissing.join(' / ')}`);
  for (const p of r.problems) console.log(`  ❌ ${p}`);
  if (r.problems.length) return false;
  console.log('  ✅ 「無い」と書いてある物は、本当に1つも在りません（説明と現物が一致）');
  return true;
}
